...
def loadImages():
    ...
    for piece in pieces:
        IMAGES[piece] = p.transform.scale(p.image.load(resource_path("images/" + piece + ".png")), (SQUARE_SIZE, SQUARE_SIZE))
            ...
            if e.type == p.QUIT:
                ...
                if e.key == p.K_o:
                    if player_one == player_two:
                        player_one = player_two = not player_two
                if e.key == p.K_ESCAPE:
                    running = False
...
def animateMove(move, screen, board, clock):
    ...
    for frame in range(frame_count + 1):
        ...
        color = [p.Color('white'), p.Color('gray')][(move.end_row + move.end_col) % 2]
